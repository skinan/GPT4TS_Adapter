...
class FFT_adapter(nn.Module):
    def __init__(self, n_layer, in_feat, seq_len):
        ...
        self.blocks = nn.ModuleList(
            [SpectBlock(in_feat, seq_len) for i in range(n_layer)]
        )
...
class CrossResidualDenseBlock(nn.Module):
    def __init__(self, input_size, cross_x_size):
        super(CrossResidualDenseBlock, self).__init__()
        self.linear1 = nn.Linear(input_size, input_size)
        self.dropout = nn.Dropout(0.3)
        self.layer_norm1 = nn.LayerNorm(input_size)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(input_size, input_size // 2)
        self.layer_norm2 = nn.LayerNorm(input_size // 2)
        self.linear3 = nn.Linear(input_size // 2, cross_x_size)
        self.layer_norm3 = nn.LayerNorm(cross_x_size)
        self.add = torch.add
        self.layer_norm_out = nn.LayerNorm(cross_x_size)

    def forward(self, x, cross_x):
        fx = self.linear1(x)
        fx = self.dropout(fx)
        fx = self.layer_norm1(fx)
        fx = self.relu(fx)
        fx = self.linear2(fx)
        fx = self.layer_norm2(fx)
        fx = self.relu(fx)
        fx = self.linear3(fx)
        fx = self.layer_norm3(fx)
        # out = self.add(fx, cross_x)
        out = fx + cross_x
...
class gpt4ts(nn.Module):
    def __init__(self, config):
        ...
        self.pred_len = 0
        self.seq_len = config["sequence_length"]  # data.max_seq_len
        self.max_len = config["sequence_length"]  # data.max_seq_len
        self.patch_size = config["patch_size"]
        self.stride = config["stride"]
        self.gpt_layers = 6
        self.feat_dim = 1  # data.feature_df.shape[1]
        self.num_classes = config["num_targets"]  # 12  # len(data.class_names)
        ...
        for i in range(self.gpt_layers):
            ...
            self.gpt2.h[i].T_adapter_gate = torch.nn.Parameter(
                torch.zeros(1, self.patch_num, 1)
            )

            self.gpt2.h[i].C_adapter = Adapter(self.d_model, self.adapter_dim)
            self.gpt2.h[i].C_num = self.feat_dim
            self.gpt2.h[i].C_adapter_gate = torch.nn.Parameter(
                torch.zeros(1, self.feat_dim, 1)
            )

        # Frequency Adapter
        self.fft_adapter = FFT_adapter(self.gpt_layers, self.feat_dim, self.patch_num)
        self.adapter_in_layer = nn.ModuleList(
            [nn.Linear(self.patch_size, self.d_model) for i in range(self.gpt_layers)]
        )
        ...
        self.out_layer = nn.Linear(
            (config["d_model"] * self.patch_num) + 384, self.num_classes
        )

    def forward(self, x_enc, x_meta):
        B, L, M = x_enc.shape
        ...
        # x1 = CrossResidualDenseBlock(outputs.size(1), x_meta.size(1))(outputs, x_meta)
        # x2 = CrossResidualDenseBlock(x_meta.size(1), outputs.size(1))(x_meta, outputs)
        # concat = torch.cat((x1, x2), dim=1)
        concat = torch.cat((outputs, x_meta), dim=1)
        outputs = self.out_layer(concat)

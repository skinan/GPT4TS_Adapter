...
class HDD_data(BaseData):
    ...
    def __init__(
        ...
        self.failed_IDs = self.all_df[
            self.all_df.failure == 1
        ].index.unique()  # IDs corresponding to assets which failed
        self.normal_IDs = sorted(
            list(set(self.all_IDs) - set(self.failed_IDs))
        )  # IDs corresponding to assets without failure
    ...
    def read_data(filepath):
        ...
        return df[
            df["model"].apply(lambda x: x.startswith("ST"))
    ...
    def process_columns(df):
        ...
        df[["capacity_bytes", "model"]] = df[["capacity_bytes", "model"]].astype(
            "category"
        )
...
class SemicondTraceData(BaseData):
    ...
    features = [
        "Actual Bias voltage (AT/CH2/RFGen/RFMatch.rMatchBias)",
        "Actual Pressure (AT/CH2/PressCtrl.rPress)",
        ...
        "Current Flow (AT/CH2/Gaspanel/Stick01.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick01/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick02.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick02/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick03.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick03/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick05.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick05/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick06.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick06/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick09.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick09/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick21.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick21/Mfc.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick22.rFlow)",
        "Current Flow (AT/CH2/Gaspanel/Stick22/Mfc.rFlow)",
        "Current Position SP Percent (AT/CH2/PressCtrl.rPosSPP)",
        "Current Power SP (AT/CH2/RFGen.rPowerSP)",
        ...
        "Next wafer src, side 1 (AT/CH2.@NextCassId01_01)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_1.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_2.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_3.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_4.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_5.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_6.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_7.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow1/Ch_8.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_1.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_2.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_3.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_4.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_5.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_6.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow2/Ch_8.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_1.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_2.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_3.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_4.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_5.rTempReading)",
        "Temp Reading  (AT/CH2/Watlow3/Ch_6.rTempReading)",
    ...
    def get_measurements(self, wafer_measurements_path):
        ...
        deprate_df1 = self.load_all(
            wafer_measurements_path,
            pattern="Rate_time_series.*_Average_",
            mode="simple",
        )
        ...
        deprate_df.loc[right_only, "mean_thickness"] = deprate_df.loc[
            right_only, "mean_thickness_right"
        ]
        ...
        roughness_df.loc[right_only, "std_thickness"] = roughness_df.loc[
            right_only, "std_thickness_right"
        ]
        ...
        assert (
            sum(measurements_df.mean_thickness.isnull()) == 0
        ), "Missing thickness measurements"
        assert (
            sum(measurements_df.std_thickness.isnull()) == 0
        ), "Missing roughness measurements"
    ...
    def get_metadata(self, catalog_path, measurements_df):
        ...
        catalog_df = catalog_df[
            catalog_df["ChamberRecipeID"].isin(
    ...
    def load_all(self, root_dir, file_list=None, pattern=None, mode=None):
        ...
        if (mode != "simple") and (self.n_proc > 1):
            ...
            if mode == "simple":
                ...
                all_df = pd.concat(
                    SemicondTraceData.load_single(path) for path in tqdm(input_paths)
                )
...
class ThingSpeakData(BaseData):
    ...
    def load_single(self, filepath):
        ...
        if self.config["task"] == "classification":
            ...
            # Define the number of repetitions (n) and total number of rows (m)
            n_repetitions = len(df.columns)
            self.max_seq_len = n_repetitions
            n_sample = len(df)
            total_rows = n_repetitions * n_sample
            df = df.stack().to_frame().rename(columns={0: "dim_0"})
            # Create the repeated index
            repeated_index = np.repeat(
                np.arange(total_rows // n_repetitions), n_repetitions
            )
            df = df.reset_index(drop=True).set_index(repeated_index)
            # Replace NaN values
            grp = df.groupby(by=df.index)
            df = grp.transform(interpolate_missing)
            # print(df)
            ...
            raise ("Only designed for classification! Other tasks not implemented !")
        return df, labels_df

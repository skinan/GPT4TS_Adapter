...
from models.gpt4ts_custom_meta import gpt4ts
...
def save_classification_report(predicted_targets, actual_targets):
    ...
    clf_df.to_csv(f"OFA_classification_report_with_meta_our_fusion.csv")
...
def main(dataframe, config):
    ...
    sequence_length = X_train.shape[1]  # Length of timeseries

    X_train = torch.Tensor(X_train)
    X_test = torch.Tensor(X_test)

    X_meta_train_embed = torch.Tensor(X_meta_train_embed)
    X_meta_test_embed = torch.Tensor(X_meta_test_embed)
    ...
    for epoch in range(num_epochs):
        ...
        optimizer.zero_grad()
        # Forward pass
        outputs = model(X_train, X_meta_train_embed)
        ...
        # model.eval()
        with torch.no_grad():
            validation_outputs = model(X_test, X_meta_test_embed)
    ...
    # print(f"Best Validation F-Score Macro: {best_val_accuracy}")
...
if __name__ == "__main__":
    ...
    dataframe = pd.read_excel(
                "/home/muhammadinan/Documents/GitHub/Multi-modal-Journal-Paper/datasets/senseurcity/SensEUR_Cleaned_NEW.xlsx"
            )
    label_dict = {
